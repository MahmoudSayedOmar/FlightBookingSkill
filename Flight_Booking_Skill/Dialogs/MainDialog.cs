...
using Flight_Booking_Skill.Responses.Main;
...
namespace Flight_Booking_Skill.Dialogs
{
    public class MainDialog : RouterDialog
    {
        ...
        protected override async Task RouteAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            localeConfig.LuisServices.TryGetValue("Flight_Booking_Skill", out var luisService);
            ...
            else
            {
                ...
                var result = await luisService.RecognizeAsync<Flight_Booking_SkillLuis>(dc.Context, CancellationToken.None);
                var intent = result?.TopIntent().intent;

                switch (intent)
                {
                    case Flight_Booking_SkillLuis.Intent.Booking:
                        {
                            turnResult = await dc.BeginDialogAsync(nameof(BookingDialog));
                            break;
                        }

                    case Flight_Booking_SkillLuis.Intent.None:
                        {
